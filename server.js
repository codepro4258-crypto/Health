...
function createApp(options = {}) {
  ...
  function serveStatic(req, res, pathname) {
    ...
    fs.readFile(fullPath, (err, data) => {
      ...
      res.writeHead(200, { 'Content-Type': types[ext] || 'text/plain', ...securityHeaders });
      res.end(data);
    });
  }
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && routineComplete) {
      ...
      db.consent
        .filter((c) => c.patientId === patientId && c.active)
        .forEach((l) => broadcastGuardian(l.guardianId, 'routine', payload));
    ...
    if (req.method === 'POST' && vitalsLog) {
      ...
      try {
        ...
        db.consent
          .filter((c) => c.patientId === patientId && c.active)
          .forEach((l) => broadcastGuardian(l.guardianId, 'vitals', { patientId, entry }));
